...
setInterval(function() {
    for(let v = 0; v<keysdown.length; v++){
        if(keysdown[v] == "ArrowLeft") cameraPos.rx += -5;
        if(keysdown[v] == "ArrowRight") cameraPos.rx += 5;
        if(keysdown[v] == "w"){ cameraPos.z += 50 }
        if(keysdown[v] == "s"){ cameraPos.z += -50 }
        if(keysdown[v] == "a"){ cameraPos.x += -50 }
        if(keysdown[v] == "d"){ cameraPos.x += 50 }
        if(keysdown[v] == "e") cameraPos.y += -30;
        if(keysdown[v] == "q") cameraPos.y += 30;
...
function regenerateFaces() {
    ...
    for(index in positions){
        ...
        colorarrside[tx+ty*worldWidth]+=25.5;
    }
    if(showDensity == false){
        for(let v = 0; v<worldWidth*worldHeight; v++){
            if(terrainFaces[v] != undefined)terrainFaces[v][5] = false
            if(sideFaces[v] != undefined) sideFaces[v][5] = false
            
        }
    } else {
    for(let v = 0; v<worldWidth*worldWidth; v++){
        let i = heightmap[v]
        ...
        let tval = colorarr[v];//Math.min(255,count(tx * terrainDetail, 0, ty*terrainDetail, tx*terrainDetail + terrainDetail, 1000, ty*terrainDetail+terrainDetail)*255/10);
        let tcolor = `rgb(${tval},${tval},${tval})`
        //if(a1!==undefined && a2!==undefined && a3!==undefined) terrainFaces[v] = [i,a1,a2,a3,tcolor];
        //let dcolor = '#000';
        //if(terrainFaces.length > v) dcolor = terrainFaces[v][4];
        ...
        sideFaces[worldWidth*worldHeight+v] = [
            {x: worldWidth*terrainDetail, y:-ty*terrainDetail, z:tx*terrainDetail+worldWidth*terrainDetail+terrainDetail},
            {x: worldWidth*terrainDetail, y:-ty*terrainDetail - terrainDetail, z:tx*terrainDetail+worldWidth*terrainDetail+terrainDetail},
            {x: worldWidth*terrainDetail, y:-ty*terrainDetail - terrainDetail, z:tx*terrainDetail + 2*terrainDetail+worldWidth*terrainDetail},
            {x: worldWidth*terrainDetail, y:-ty*terrainDetail, z:tx*terrainDetail + terrainDetail+worldWidth*terrainDetail+terrainDetail},
            `rgb(${255-tx*255/worldWidth},${ty*255/worldWidth},${tx*255/worldWidth})`,true]
