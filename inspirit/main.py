...
def deepToString(arr):
    return '['+",".join(toString(x) for x in arr)+']'
...
for j in range(len(frameCounts)):
    for i in range(len(frameCounts[j])):
        frameCounts[j][i] = [frameCounts[j][i].xmin, frameCounts[j][i].ymin, frameCounts[j][i].xmax, frameCounts[j][i].ymax, frameCounts[j][i].objness, '['+toString(frameCounts[j][i].classes)+']']
print(labels)
print(toString(frameCounts[len(frameCounts) - 1]))
